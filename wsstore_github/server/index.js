...
function readDB() {
  if (!fs.existsSync(DB_PATH)) {
    const seed = {
      products: [
        { id: '1', name: 'Sample Product A', category: 'Featured', desc: 'Edit this in your admin panel.', price: 100, currency: 'USD', badge: 'New', available: true, emoji: '🌟', createdAt: Date.now() },
        { id: '2', name: 'Sample Product B', category: 'Standard', desc: 'Edit this in your admin panel.', price: 50,  currency: 'USD', badge: '',    available: true, emoji: '💎', createdAt: Date.now() }
      ],
      activities: [],
      comments: []
    };
...
app.post('/api/admin/products', auth, (req, res) => {
  const db = readDB();
  const p = { id: crypto.randomUUID(), ...req.body, createdAt: Date.now() };
  db.products.push(p);
  writeDB(db);
...
app.put('/api/admin/products/:id', auth, (req, res) => {
  ...
  writeDB(db);
  log('EDIT', `Edited: ${db.products[i].name} — $${db.products[i].price}`, req.ip);
